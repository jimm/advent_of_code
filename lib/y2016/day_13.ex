defmodule Y2016.Day13 do
  ...
  defp dist(map, fnum, visited, {x, y} = curr_loc, goal_loc, num_steps) do
    ...
    if wall?(new_map, curr_loc) do
      ...
      |> Enum.map(&(dist(new_map, fnum, Map.put(visited, curr_loc, true),
                  &1, goal_loc, num_steps+1)))
      |> Enum.min
    end
  end
  ...
  defp reachable_in(map, fnum, visited, recently_added, steps_remaining) do
    IO.puts ""                                              # DEBUG
    steps_remaining |> IO.inspect(label: "steps_remaining") # DEBUG
    recently_added |> IO.inspect(label: "recently_added")   # DEBUG
